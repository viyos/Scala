...
def main(args: Array[String]) = {
   ...
   println( "Chaves : " + cores.keys ) //Chaves são os valores que estão sendo referidos
   println( "Valores : " + cores.values ) //Valores são os codigos que foram referidos
   println( "Verifca se o mapa de cores está vazio : " + cores.isEmpty )

   println("Exemplo 2:")

   cores.keys.foreach{ i => 
      print("Chave = " + i ) //Imprime a chave somente o i
      println(", Valor = " + cores(i)) //Imprime o valor somente o i
   }
