...
def main(args: Array[String]) = {
   ...
   while (fSource1.hasNext) //função hasNext caso tenha um proximo caracter ele irá imprimir o item
   {
   ...
   for(line <- fSource2.getLines) //função getLines ele pega linha por linha do arquivo
   { 
      println(line) 
   } 
